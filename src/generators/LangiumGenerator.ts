...
import { VAttribute, VClass, VModel, VPackage, VReference } from '../language-server/generated/ast';
...
export function generateLangium(model: VModel, filePath: string, destination: string | undefined): string {
  ...
  fileNode.append("// automatically generated by Vmsc", NL, "//Please report any bug or improvement as an issues in https://github.com/othy1010/VMSC", NL, NL, `grammar ${vpackage.name.slice(1, -1)}`, NL, NL, 'entry ');
...
function generateReference(e: VReference): string {
  return `${e.name} ${generateReferenceType(e.VType, e.UpperBound!)}`;
}
...
function generateReferenceType(e: Reference<VClass>, upperBound: number): string {
  const name = e.$nodeDescription?.name.substring(1, e.$nodeDescription?.name.length - 1)
  if (upperBound == -1) {
    return `"(" ${name} += ${generateReferenceType(e, 0)} ( "," ${name} += ${generateReferenceType(e, 0)})* ")"`;
  } else if (upperBound == 0) {
    return `${e.$nodeDescription?.name.substring(1, e.$nodeDescription?.name.length - 1)} | ID  `;
  }
  return `${name}=${e.$nodeDescription?.name.substring(1, e.$nodeDescription?.name.length - 1)} | ID`;
}
